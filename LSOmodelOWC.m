function [spOut, vOut] = LSOmodelOWC(spEx, spIn, DT, Iext)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Original Wang-Colburn Model of Lateral Superior Olive 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Input 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Notes on the model 
%  The model was originally created by Wang & Colburn (2012), based 
%  on the Rothman-Manis (2003) model. The parameter values of the model 
%  membrane are the same as those presented in Wang & Colburn (2012), 
%  except for the sodium reversal potential ENa, which had to be revised 
%  from 0 mV to +50 mV to replicate their original results. The maximum 
%  conductance for the low-voltage-activated potassium conductance gKL is 
%  fixed to 85 nS in this code. More detailed descriptions and analyses 
%  of the model can be found in Ashida et al. (2017). 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% References 
...
% membrane parameters
Cs  = 31.4e-6; % 31.4*1e-6[uF] = 31.4[pF] 
gLL = 31.4e-6; % 31.4*1e-6[mS] = 31.4[nS] 
gKL = 85.0e-6; % 85.0*1e-6[mS] = 85.0[nS]
gKH = 1200.0e-6; % 1200*1e-6[mS] = 1200[nS] 
gNa = 8000.0e-6; % 8000*1e-6[mS] = 8000[nS] 
EL = -65.0; % [mV] leak reversal potential 
EK = -70.0; % [mV] potassium reversal potential 
...
Ein =-70.0; % [mV] inhibitory synaptic input reversal potential  
...
function x = winf(v) % KLVA activation : steady state
 x = 1.0 / sqrt(sqrt( 1.0 + exp( -(v+48.0)/6.0 ) )); 
end
function x = tauw(v) % KLVA activation : time constant 
 x = 1.5 + 100.0 / ( 6.0*exp( (v+60.0)/6.0 ) + 16.0*exp( -(v+60.0)/45.0 ) );
end
function x = zinf(v) % KLVA inactivation : steady state
 x = 0.5 + 0.5 / ( 1.0 + exp( (v+71.0)/10.0 ) ); 
end
function x = tauz(v) % KLVA inactivation : time constant 
 x = 50.0 + 1000.0 / ( exp( (v+60.0)/20.0 ) + exp( -(v+60.0)/8.0 ) );
end
function x = ninf(v) % KHVA activation 1 : steady state
 x = 1.0 / sqrt( 1.0 + exp( -(v+15.0)/5.0 ) ); 
end
function x = taun(v) % KHVA activation 1 : time constant 
 x = 0.7 + 100.0 / ( 11.0*exp( (v+60.0)/24.0 ) + 21.0*exp( -(v+60.0)/23.0 ) );
end
function x = pinf(v) % KHVA activation 2 : steady state
 x = 1.0 / ( 1.0 + exp( -(v+23.0)/6.0 ) ); 
end
function x = taup(v) % KHVA activation 2 : time constant 
 x = 5.0 + 100.0 / ( 4.0*exp( (v+60.0)/32.0 ) + 5.0*exp( -(v+60.0)/22.0 ) );
end
function x = minf(v) % Na activation : steady state
 x = 1.0 / ( 1.0 + exp( -(v+38.0)/7.0 ) ); 
end
function x = taum(v) % Na activation : time constant 
 x = 0.04 + 10.0 / ( 5.0*exp( (v+60.0)/18.0 ) + 36.0*exp( -(v+60.0)/25.0) );
end
function x = hinf(v) % Na inactivation : steady state
 x = 1.0 / ( 1.0 + exp( (v+65.0)/6.0 ) ); 
end 
function x = tauh(v) % Na inactivation : time constant 
